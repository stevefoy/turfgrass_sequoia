...
def sequoia_vignetteCorrection(meta, imageRaw):
...
def sequoia_irradiance(meta, imageRaw):

    # get radiometric calibration factors (Sequoia Firmware Version 1.2.1 or later)
    sensorModel = meta.get('XMP:SensorModel').split(',')
...
def vignette_correction(meta, xDim, yDim):
    polynomial2DName = meta.get('XMP:VignettingPolynomial2DName')
    polynomial2D = meta.get('XMP:VignettingPolynomial2D')
...
def GetTimefromStart(meta):
    Time = datetime.datetime.strptime(meta.get('Composite:SubSecCreateDate'), "%Y:%m:%d %H:%M:%S.%f")
...
def sunIrradiance(meta):
    encoded = meta.get('XMP:IrradianceList')
    # decode the string
...
def correct_lens_distortion_sequoia(meta, image):
    ...
    FisheyePoly = np.array(meta.get('XMP:FisheyePolynomial').split(',')).astype(float)
    p0 = FisheyePoly[0]
    p1 = FisheyePoly[1]
    p2 = FisheyePoly[2]
    p3 = FisheyePoly[3]

    FisheyeAffineMat = np.array(meta.get('XMP:FisheyeAffineMatrix').split(',')).astype(float)
    ...
    FocalLength = float(meta.get('EXIF:FocalLength'))
    h, w = image.shape

    # get the two principal points
    pp = np.array(meta.get('XMP:PrincipalPoint').split(',')).astype(float)
    # values in pp and focallength are in [mm] and need to be rescaled to pixels
    FocalPlaneXResolution = float(meta.get('EXIF:FocalPlaneXResolution'))
    FocalPlaneYResolution = float(meta.get('EXIF:FocalPlaneYResolution'))
