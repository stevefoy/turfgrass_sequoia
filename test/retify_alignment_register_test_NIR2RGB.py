...
Saved homography matrix to align_NIR2RGB.json
Scale X: 0.9730766382569641, Scale Y: 0.9716298758880888
Rotation (degrees): 0.548954259297523
Translation: [ 16.41252668 -19.55590887   1.        ]
Mean pixel difference between images: 113.64865056818182
Mean re-projection error: 0.8898089528083801
Maximum re-projection error: 2.9193155765533447
...
def main(img1_path, img2_path, pattern_size, roi1, roi2, homography_file):
    img1 = load_image(img1_path)
    img2 = load_image(img2_path)

    print(f"Loaded Image 1 shape: {img1.shape}")
    print(f"Loaded Image 2 shape: {img2.shape}")

    # Apply the ROI to both images
    img1 = apply_roi(img1, roi1)
    img2 = apply_roi(img2, roi2)

    # Debug: visualize the ROI applied images
    plt.figure(figsize=(12, 6))
    plt.subplot(1, 2, 1)
    plt.title('Image 1 with ROI')
    plt.imshow(cv2.cvtColor(img1, cv2.COLOR_BGR2RGB) if len(img1.shape) == 3 else img1, cmap='gray')
    plt.subplot(1, 2, 2)
    plt.title('Image 2 with ROI')
    plt.imshow(cv2.cvtColor(img2, cv2.COLOR_BGR2RGB) if len(img2.shape) == 3 else img2, cmap='gray')
    plt.show()

    # Find checkerboard corners
    image = preprocess_nir_image(img1)
    ret1, corners1, gray1 = find_checkerboard_corners(image, pattern_size)
    
    image = enhance_image(img2)
    ret2, corners2, gray2 = find_checkerboard_corners(image, pattern_size)
    ...
    plt.figure(figsize=(12, 6))
    plt.subplot(1, 2, 1)
    plt.title('Checkerboard Detection - Image 1')
    plt.imshow(cv2.cvtColor(img1, cv2.COLOR_BGR2RGB) if len(img1.shape) == 3 else img1, cmap='gray')
    plt.subplot(1, 2, 2)
    plt.title('Checkerboard Detection - Image 2')
    plt.imshow(cv2.cvtColor(img2, cv2.COLOR_BGR2RGB) if len(img2.shape) == 3 else img2, cmap='gray')
    plt.show()
    ...
    aligned_img2, H = align_images(img1, img2, corners1, corners2)
    ...
    if aligned_img2.shape != img1.shape:
        aligned_img2 = cv2.resize(aligned_img2, (img1.shape[1], img1.shape[0]))

    if len(aligned_img2.shape) == 2:
        aligned_img2 = cv2.cvtColor(aligned_img2, cv2.COLOR_GRAY2BGR)
    if len(img1.shape) == 2:
        img1 = cv2.cvtColor(img1, cv2.COLOR_GRAY2BGR)
    
    # Calculate the absolute difference between the two images
    diff_image = cv2.absdiff(img1, aligned_img2)
    ...
    mean_reproj_error, max_reproj_error, reproj_errors = calculate_reprojection_error(corners1, corners2_proj)
    ...
    plt.imshow(cv2.cvtColor(aligned_img2, cv2.COLOR_BGR2RGB) if len(aligned_img2.shape) == 3 else aligned_img2, cmap='gray')
...
if __name__ == "__main__":
    ...
    homography_file = 'align_NIR2RGB.json'
    
    # Example with NIR as reference (img1_is_ref=False)
    main(img_ref_path, img_path , pattern_size, roi_nir, roi_rgb, homography_file)
