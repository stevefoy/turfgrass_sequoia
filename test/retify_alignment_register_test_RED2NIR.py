...
"""
Saved homography matrix to align_RED2NIR.json
Scale X: 1.0004732403887147, Scale Y: 1.0037915764273486
Rotation (degrees): 0.012811463843117935
Translation: [ 3.4605307  22.63208507  1.        ]
Mean pixel difference between images: 7.198087993421052
Mean re-projection error: 0.2137371301651001
Maximum re-projection error: 0.6176570057868958
...
def main(nir_img_path, r_img_path, pattern_size, roi_nir, roi_rgb, homography_file):
    nir_img = load_image(nir_img_path)
    r_img = load_image(r_img_path)

    print(f"Loaded NIR image shape: {nir_img.shape}")
    print(f"Loaded RGB image shape: {r_img.shape}")

    # Apply the ROI to both images
    nir_img = apply_roi(nir_img, roi_nir)
    r_img = apply_roi(r_img, roi_rgb)
    ...
    plt.imshow(cv2.cvtColor(r_img, cv2.COLOR_BGR2RGB) if len(r_img.shape) == 3 else r_img, cmap='gray')
    plt.show()

    # Find checkerboard corners
    ret_nir, corners_nir, gray_nir = find_checkerboard_corners(nir_img, pattern_size, is_nir=True)
    ret_r, corners_rgb, gray_rgb = find_checkerboard_corners(r_img, pattern_size, is_nir=False)

    # Debug: visualize checkerboard detection
    if ret_nir:
        cv2.drawChessboardCorners(nir_img, pattern_size, corners_nir, ret_nir)
    if ret_r:
        cv2.drawChessboardCorners(r_img, pattern_size, corners_rgb, ret_r)
    ...
    if aligned_r_img.shape != nir_img.shape:
        aligned_r_img = cv2.resize(aligned_r_img, (nir_img.shape[1], nir_img.shape[0]))

    if len(aligned_r_img.shape) == 2:
        aligned_r_img = cv2.cvtColor(aligned_r_img, cv2.COLOR_GRAY2BGR)
    if len(nir_img.shape) == 2:
        nir_img = cv2.cvtColor(nir_img, cv2.COLOR_GRAY2BGR)
    
    # Calculate the absolute difference between the two images
    diff_image = cv2.absdiff(nir_img, aligned_r_img)
    ...
    plt.subplot(2, 2, 2)
    plt.title('RED Image (Aligned)')
    plt.imshow(cv2.cvtColor(aligned_r_img, cv2.COLOR_BGR2RGB) if len(aligned_r_img.shape) == 3 else aligned_r_img, cmap='gray')
...
if __name__ == "__main__":
    ...
    red_img_path = '/media/freddy/vault/datasets/Parrot/zendo_release/processed/Calibration/0020/IMG_240711_094018_0000_RED.TIF'
    pattern_size = (12, 8)
    roi_nir = (0, 10, 1280, 960)  # Example ROI coordinates (x, y, width, height)
    roi_rgb = (0, 10, 1280, 960)  # Example ROI coordinates (x, y, width, height)
    homography_file = 'align_RED2NIR.json'
    main(nir_img_path, red_img_path, pattern_size, roi_nir, roi_rgb, homography_file)
